...
Taking from Reddit:
    ...
    The question instructs us to multiply each empty row by 1000000, which is equivalent to adding 999999 new rows.
...
def exsub(p, q, h=True):
    ...
    for item in exp:
        ...
        if maxin < item:
            right -= 1
    return abs(minin - maxin) + (len(exp[left:right]) * 999999)
